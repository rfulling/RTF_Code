...
{   
    ...
    var CI_CUSTOMERS  = objContext.getSetting('SCRIPT','custscript_ci_customers');
    ...
    for(var i=0; i<arrCustomers.length; i++)
    {
        ...
        var stCustomerId      = arrCustomers[i];
        var arrInvResult      = getAllCustomersInvoices(stCustomerId,CI_START_DATE,CI_END_DATE);
            ...
            if(stRelInvoices)
            {
                ...
                for(var x=1; x<=arrRelInvoices.length; x++)
                {
                    ...
                    for(var y=1; y<=lineCount; y++)
                    {
                        ...
                        var qty  = nlapiEscapeXML(recInvoice.getLineItemValue('item','quantity',y));
...
{
    var arrHead = '<tr>';
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" border-left="0pt" width="10%" align="center">REF NO</td>';					
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" width="20%" align="center">ITEM</td>';
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" width="20%" align="center">DESCRIPTION</td>';
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" width="20%" align="center">MEMO</td>';
        arrHead +=      '<td class="header" border-top="0pt" border-right="0pt" width="10%" align="center">QTY</td>';
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" width="10%" align="center">UNIT PRICE</td>';
        arrHead +=		'<td class="header" border-top="0pt" border-right="0pt" width="10%" align="center">AMOUNT</td>';
        arrHead += '</tr>';   
...
function buildRowPerAddress(arrRowContent)
{
    ...
    for(var i=0; arrRowContent && i<arrRowContent.length; i++)
    {
        ...
        var fQty    = (parseFloat(arrRowContent[i][3])).toFixed(2);
        var fRate   = (arrRowContent[i][4]) ? formatCurrency(arrRowContent[i][4],2) : '';
        var fAmount = formatCurrency(arrRowContent[i][5],2);
        
        arrRow +='<tr>'
		arrRow +=	'<td class="itemline" width="10%" align="center"> ' + stRefno + '&#160;&#160;</td>';
		arrRow +=	'<td class="itemline" width="20%" align="left">  ' + stItem  + '&#160;&#160;</td>';
		arrRow +=	'<td class="itemline" width="20%" align="left">  ' + stDesc  + '&#160;&#160;</td>';
        arrRow +=	'<td class="itemline" width="20%" align="left"> &#160;&#160;</td>';
		arrRow +=	'<td class="itemline" width="10%"  align="right"> ' + fQty    + '&#160;&#160;</td>';
		arrRow +=	'<td class="itemline" width="10%" align="right"> ' + fRate   + '&#160;&#160;</td>';
		arrRow +=	'<td class="itemline" width="10%" align="right"> ' + fAmount + '&#160;&#160;</td>';
		arrRow +='</tr>';
    }
...
{    
    ...
    var arrGrandTotal = ['<tr>',
                			...
                			'</tr>',
                            '<tr>',
        						'<td class="itemline" colspan="4" width="20%">&#160;</td>',        						
...
function getAllCustomersInvoices(customer,startdate,enddate)
{
    var arrInvoiceFilter = [new nlobjSearchFilter('trandate',null,'within',startdate,enddate),
                            new nlobjSearchFilter('internalid','customer','anyof',customer)];
...
function setAddressMargin(stShipToAddr,record,type)
{
    ...
    var stDfaultBillAddrs = stShipToAddr;
