	...
	function(search,message) {
		...
		function lineInit(context){
			...
			if (context.sublistId == 'item'){
				...
				try{
                   //  log.debug('here on lineInit ', 'lineInit');
		...
		function validateLine(context){
			//todo here make this available for other transactions by making the sublistId dynamic
			var curRec = context.currentRecord;
			var sublistID = 'item'
			switch(curRec.type ){
		...
		function validateDept(departmentID,ccId){
			var validClass = false;
			var classificationSearchObj = search.create({
				   type: "classification",
				   filters:
				   [
				      ["custrecord_tw_depart_class","anyof",departmentID],
				      "AND",
				      ['internalid', "anyof", ccId],
				   ],
				   columns:
				   [
				      search.createColumn({
